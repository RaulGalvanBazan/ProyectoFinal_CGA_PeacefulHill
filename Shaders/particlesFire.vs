...
uniform vec3 Gravity = vec3(0.0,0.1,0.8); 
...
vec3 randomInitialVelocity() {
    float velocity = mix(0.1, 1.5, texelFetch(RandomTex, 2 * gl_VertexID, 0).r );
...
vec3 randomInitialVelocity2() {
    float theta = mix(0.0, PI/10, texelFetch(RandomTex, 3 * gl_VertexID, 0).r);
    float phi = mix(0.0, PI, texelFetch(RandomTex, 3 * gl_VertexID + 1, 0).r);
    float velocity = mix(0.6, 0.8, texelFetch(RandomTex, 3 * gl_VertexID + 2, 0).r);
    vec3 v = vec3(sin(theta) * cos(phi), 0.0, sin(theta) * sin(phi));
    return normalize(EmitterBasis * v) * 6.5 * velocity;
}
...
void main()
{
	if(Pass == 1){
	    ...
	    if( VertexAge < 0 || VertexAge > ParticleLifetime ) {
	        ...
	        Position = VertexPosition + VertexVelocity * DeltaT + Gravity * DeltaT;
